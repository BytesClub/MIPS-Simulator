    ...
    parseTokens(instructions) {
        ...
        instructions.forEach((item, index) => {
            ...
            if (token === ".data" || token === ".text") {
                ...
                // If type not found
                if (type === null) {
                    console.error(`Missing type specification for instruction at ${line}. Check mannual for details.`);
                    exit(3);
                }
                // If it is a label
                if (checkType(token, "label")) {
                    ...
                    let value = type === "Data Declaration" && lexer.length === 2 ? {
                        encoding: mapToEncoding(lexer.shift()),
                    ...
                    // Already exists
                    if (symIndex !== -1 && symTable.hasOwnProperty(symIndex) && symTable[symIndex].value === "undefined") {
                        let prevTable = symTable.slice(0, symIndex).push(labelDef);
                        let nextTable = symTable.slice(symIndex + 1);
                        symTable = prevTable.concat(nextTable);
                    } else {
                    ...
                    if (typeof type !== "undefined" && type !== null) {
                        ...
                        for (let i = 0; i < type.arguments; i++) {
                            let arg = lexer.shift();
                            let argument_type = argumentType.shift();
                            if (checkType(arg, argument_type)) {
                                args.push(arg);
                            } else {
                                // Invalid argument
                                console.error(`Invalid argument provided with ${type.action} instruction; Expected ${argument_type}, found ${arg} at line ${line}`);
                                exit(3);
                            }
                        }
                        type.argument = args;
