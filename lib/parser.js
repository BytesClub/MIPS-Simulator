        ...
        instructions.map((item, index) => {
            ...
            if (token === '.data' || token === '.text') {
                ...
                if (LabelRegex.test(token)) {
                    ...
                    let type = this.isInstruction(token)
                    if (typeof type !== 'undefined' && type !== null) {
                        let args = [];
                        if (lexer.length !== type.arguments) {
                            console.error(`Invalid number of arguments provided with ${type.action} instruction.`);
                            exit(3);
                        }
                        for (let i = 0; i < type.arguments; i++) {
                            args.push(lexer.shift());
                        }
                        type.argument = args;
            ...
            console.log(`After ${index} iteration:`);
            console.log("SymbolTable: ", symTable);
            console.log("AbstractSyntaxtTree: ", AST);
        });
        this.AST      = AST;
