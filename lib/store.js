...
class Store extends Object {
    constructor(props) {
        ...
        let isSilent = false, showWarn = true, showDetail = false;
        if (typeof flag !== 'undefined') {
            const { silent, warning, details } = flag;
            isSilent   = (typeof silent === 'boolean') ? silent : false;
            showWarn   = (typeof warning === 'boolean') ? warning : true;
            showDetail = (typeof details === 'boolean') ? details : false;
        }
    ...
    save(content) {
        ...
        if (store.existsSync(file)) {
            ...
            try {
                store.accessSync(file, store.constants.W_OK); // Bug: Not working
            } catch (e) {
                this.hasError = true;
