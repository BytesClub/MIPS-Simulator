...
class VM extends Object {
    constructor(props) {
        ...
        // %k0-1, $gp, $sp, $fp, $ra, $f0-$f3, $f4-$f10, $f12-$f14, $f16-$f18, $f20-$f31
    ...
    execute() {
        ...
        // Increment PC by 1, execute current instruction
    ...
    valueFromRegister(register, line) {
        ...
        if (! regObj.hasOwnProperty("value") || (typeof value === "number" &&
            isNaN(regObj.value))) {
            // Invalid value found
    ...
    syscall(line) {
        ...
        switch (flagVariable) {
            case 1:
                ...
                break;
            case 4:
                stdout.write(`${this.valueFromRegister("$a0")}`, "ASCII");
                break;
            case 10:
