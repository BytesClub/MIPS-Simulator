    ...
    add(item, index) {
        switch (item.type) {
            case 'Register':
                ...
                this.loadOnRegister(destr, (value1 + value2), index);
                break;
            case 'Immidiate':
                const [ desti, src ] = item.argument;
                const valuer = this.valueFromRegister(src, index);
                const valuei = this.valueFromRegister(desti, index);
                this.loadOnRegister(desti, (valuer + valuei), index);
                break;
            default:
                console.error(`Error: Inavlid add statement found at ${index}`);
                exit(4);
        }
    }
    ...
    syscall() {
        ...
        switch (Number(register["$v0"])) {
            case 1:
                ...
                if (isNaN(value)) {
                    console.error("Value Error: Expected integer, recieved something else!");
                    exit(4);
                } else {
