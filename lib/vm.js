...
class VM extends Object {
    ...
    execute() {
        ...
        const key = InstructionSet.slice(ProgramCounter, ProgramCounter + 1).shift();

        // Validity of instruction
        if (typeof key === "undefined" || key === null) {
            const counter = Number(ProgramCounter * 4 + 0x10000).toString(16).substr(-4);
    ...
    load(item, i) {
        switch (item.type) {
            case "Integer":
                ...
                this.loadOnRegister(desti, Number(integer), i);
                break;
            case "Address":
                const [ desta, label ] = item.argument;
                const value = this.valueFromLabel(label, i);
                this.loadOnRegister(desta, value, i);
                break;
            case "Register":
                const [ destr, srcr, ofs ] = item.argument;
                const offset = typeof ofs !== "undefined" ?
                               Number(ofs) :
                               0;
                let addr     = Number(this.valueFromRegister(srcr, i));
                const target = offset !== 0 ?
                               addr + Number(offset) :
                               addr;
                const valuer = this.valueFromLabel(target, i);
                this.loadOnRegister(destr, valuer, i);
                break;
            default:
    ...
    loadOnRegister(register, value, i) {
        ...
        if (typeof regObj === "undefined" || regObj === null) {
            ...
            console.error(`Fatal Error: Invalid register ${register} is given, please check at the line number ${i}`);
            exit(4);
        }
        if (regObj.name === "$zero") {
            // $r0/$zero is a readonly register
            console.error(`Fatal Error: Cannot overwrite zero register, please check at the line number ${i}`);
            exit(4);
        }
        if (typeof value === "number") {
            ...
            if (isNaN(value)) {
                // Invalid value given
                console.error(`Type Error: Type mismatch while storing into register, expected integer at line ${i}`);
                exit(4);
            }
            // Update the value
            regObj.value = value;
    ...
    valueFromRegister(register, i) {
        ...
        if (! regObj.hasOwnProperty("value")) {
            ...
            console.error(`Value Error: Un-initialized value found in ${register}, please check at the line number ${i}`);
            exit(4);
        }
        if (typeof value === "number" && isNaN(regObj.value)) {
            // Invalid value found
            console.error(`Value Error: Invalid value found in ${register}, please check at the line number ${i}`);
            exit(4);
        }
        // Return the value
        return regObj.value;
    ...
    encodeFromRegister(register, i) {
        ...
        if (! regObj.hasOwnProperty("encoding")) {
            ...
            console.error(`Type Error: Non string literal found in ${register}, please check at the line number ${i}`);
            exit(4);
        }
        // Return the encoding
    ...
    valueFromLabel(label, i) {
        ...
        if (typeof id === "undefined" || id === null) {
            ...
            console.error(`Reference Error: Cannot find label ${label} at line ${i}`);
            exit(4);
        }
        // Default return
        return id.value;
    ...
    store(item, i) {
        switch (item.type) {
            case "Register":
                const [ valuer, destr, ofs ] = item.argument;
                const value  = this.valueFromRegister(valuer, i);
                const offset = typeof ofs !== "undefined" ?
                               Number(ofs) :
                               0;
                let addr     = Number(this.valueFromRegister(destr, i));
                const target = offset !== 0 ?
                               addr + Number(offset) :
                               addr;
                ...
                break;
            case "Immidiate":
                const [ valuei, desti ] = item.argument;
                this.memory(desti, valuei, i);
                break;
            default:
    ...
    memory(target, value, i) {
        let entry = this.AddressTable.find(e => e.name === target);
        if (typeof entry !== "undefined" && entry.readOnly === true) {
            console.error(`Error: Access to read only memory region. Permission denied at ${i}`);
            exit(4);
        }
