...
class VM extends Object {
    ...
    run(parseTree) {
        ...
        if (typeof loader !== "undefined" || loader !== null) {
            ...
            while (this.shouldRun && this.ProgramCounter < this.fenceSize) {
                this.execute();
            }
        }
    }
    ...
    valueFromRegister(register, line) {
        console.log(`209: valueFromRegister: Register: ${register}`);
    ...
    divide(item, index) {
        switch (item.type) {
            case 'Register':
                const [ src1, src2 ] = item.argument;
                console.log(`297: Division: ${src1} and ${src2}`);
                const value1 = this.valueFromRegister(src1, index);
                const value2 = this.valueFromRegister(src2, index);
                this.loadOnRegister("$lo", parseInt((value1 / value2), 10), index);
    ...
    /*
     * Branch Prediction Module [start]
     */
    branch(item, index) {
        switch (item.type) {
            case 'Equality':
